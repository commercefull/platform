...
describe('Content Pages API', () => {
  ...
  describe('POST /content/pages/:id/duplicate', () => {
    it('should duplicate a page', async () => {
      if (!createdPageId) return;

      const duplicateData = {
        title: `Duplicated Page ${Date.now()}`,
        slug: `duplicated-page-${Date.now()}`
      };

      const response = await client.post(`${API_BASE}/pages/${createdPageId}/duplicate`, duplicateData);
      ...
      // Cleanup duplicated page
      if (response.data.data.id) {
        await client.delete(`${API_BASE}/pages/${response.data.data.id}`);
      }
    });
  });

  describe('GET /content/pages/:id/full', () => {
    it('should return a page with all blocks', async () => {
      if (!createdPageId) return;

      const response = await client.get(`${API_BASE}/pages/${createdPageId}/full`);
  ...
  describe('DELETE /content/pages/:id', () => {
    it('should delete a page', async () => {
      if (!createdPageId) return;

      const response = await client.delete(`${API_BASE}/pages/${createdPageId}`);

      expect(response.status).toBe(200);
      expect(response.data.success).toBe(true);
      createdPageId = ''; // Clear so afterAll doesn't try to delete again
    });
  });
});
