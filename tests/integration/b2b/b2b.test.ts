...
import { TEST_B2B_COMPANY_ID, TEST_B2B_QUOTE_ID, ADMIN_CREDENTIALS } from '../testConstants';
...
const createTestQuote = (companyId: string, overrides: Partial<any> = {}) => ({
  ...
  items: [{ productId: 'prod-001', quantity: 10, customPrice: 99.99 }],
...
describe('B2B Feature Tests', () => {
  ...
  afterAll(async () => {
    ...
    for (const id of createdResources.companyIds) {
      if (!seededIds.includes(id)) {
        try {
          await client.delete(`/business/b2b/companies/${id}`, { headers });
        } catch (e) {}
      }
    }
    for (const id of createdResources.quoteIds) {
      if (!seededIds.includes(id)) {
        try {
          await client.delete(`/business/b2b/quotes/${id}`, { headers });
        } catch (e) {}
      }
    }
    for (const id of createdResources.workflowIds) {
      try {
        await client.delete(`/business/b2b/workflows/${id}`, { headers });
      } catch (e) {}
    }
  });
  ...
  describe('Company Management', () => {
    ...
    it('UC-B2B-005: should approve a company', async () => {
      const response = await client.post(
        `/business/b2b/companies/${testCompanyId}/approve`,
        {},
        {
          headers: { Authorization: `Bearer ${adminToken}` },
        },
    ...
    it('UC-B2B-006: should suspend a company', async () => {
      const response = await client.post(
        `/business/b2b/companies/${testCompanyId}/suspend`,
        {
  ...
  describe('Quote Management', () => {
    ...
    it('UC-B2B-020: should send a quote', async () => {
      const response = await client.post(
        `/business/b2b/quotes/${testQuoteId}/send`,
        {},
        {
          headers: { Authorization: `Bearer ${adminToken}` },
        },
