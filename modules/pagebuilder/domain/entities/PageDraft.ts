...
export class PageDraft {
  ...
  static create(
    props: Omit<PageDraftProps, 'status' | 'blocks' | 'version' | 'createdAt' | 'updatedAt'> & {
      blocks?: PlacedBlockProps[];
      status?: DraftStatus;
    },
  ): PageDraft {
  ...
  getBlocksByRegion(region: string): PlacedBlockProps[] {
    return this.props.blocks.filter(b => b.region === region && !b.parentBlockId).sort((a, b) => a.order - b.order);
  }
  ...
  getChildBlocks(parentBlockId: string): PlacedBlockProps[] {
    return this.props.blocks.filter(b => b.parentBlockId === parentBlockId).sort((a, b) => a.order - b.order);
  }
  ...
  private getNextOrder(region: string, parentBlockId?: string): number {
    const siblings = this.props.blocks.filter(b => b.region === region && b.parentBlockId === parentBlockId);
