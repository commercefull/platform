...
export async function findByMerchant(merchantId: string): Promise<MerchantShippingTemplate[]> {
  return (
    (await query<MerchantShippingTemplate[]>(
...
export async function create(
  params: Omit<MerchantShippingTemplate, 'merchantShippingTemplateId' | 'createdAt' | 'updatedAt' | 'deletedAt'>,
): Promise<MerchantShippingTemplate | null> {
  const now = new Date();
  return queryOne<MerchantShippingTemplate>(
...
export async function update(
  merchantShippingTemplateId: string,
  params: Partial<Omit<MerchantShippingTemplate, 'merchantShippingTemplateId' | 'merchantId' | 'createdAt' | 'updatedAt' | 'deletedAt'>>,
): Promise<MerchantShippingTemplate | null> {
  const now = new Date();
  ...
  if (params.estimatedDays !== undefined) {
    fields.push(`"estimatedDays" = $${idx++}`);
    values.push(params.estimatedDays);
  }
  if (params.baseRate !== undefined) {
    fields.push(`"baseRate" = $${idx++}`);
    values.push(params.baseRate);
  }
...
export async function softDelete(merchantShippingTemplateId: string): Promise<void> {
  await query(`UPDATE "merchantShippingTemplate" SET "deletedAt" = $1, "updatedAt" = $2 WHERE "merchantShippingTemplateId" = $3`, [
    new Date(),
    new Date(),
    merchantShippingTemplateId,
  ]);
