...
export async function create(
  params: Omit<MerchantTransaction, 'merchantTransactionId' | 'createdAt'>,
...
export async function findByMerchant(merchantId: string, limit = 50, offset = 0): Promise<MerchantTransaction[]> {
  return (
    (await query<MerchantTransaction[]>(
...
export async function findByReference(referenceId: string, referenceType: string): Promise<MerchantTransaction[]> {
  return (
    (await query<MerchantTransaction[]>(`SELECT * FROM "merchantTransaction" WHERE "referenceId" = $1 AND "referenceType" = $2`, [
      referenceId,
      referenceType,
    ])) || []
  );
