...
export async function create(
  params: Omit<MerchantInvoice, 'merchantInvoiceId' | 'createdAt' | 'updatedAt'>,
...
export async function markPaid(merchantInvoiceId: string): Promise<void> {
  const now = new Date();
  await query(`UPDATE "merchantInvoice" SET status = 'paid', "paidAt" = $1, "updatedAt" = $2 WHERE "merchantInvoiceId" = $3`, [
    now,
    now,
    merchantInvoiceId,
  ]);
