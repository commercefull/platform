...
export async function findByMerchant(merchantId: string, limit = 20, offset = 0): Promise<MerchantPayout[]> {
  return (
    (await query<MerchantPayout[]>(`SELECT * FROM "merchantPayout" WHERE "merchantId" = $1 ORDER BY "createdAt" DESC LIMIT $2 OFFSET $3`, [
      merchantId,
      limit,
      offset,
    ])) || []
...
export async function findById(merchantPayoutId: string): Promise<MerchantPayout | null> {
  return queryOne<MerchantPayout>(`SELECT * FROM "merchantPayout" WHERE "merchantPayoutId" = $1`, [merchantPayoutId]);
}
...
export async function updateStatus(merchantPayoutId: string, status: string, processedAt?: Date): Promise<void> {
  await query(`UPDATE "merchantPayout" SET status = $1, "processedAt" = $2, "updatedAt" = $3 WHERE "merchantPayoutId" = $4`, [
    status,
    processedAt || null,
    new Date(),
    merchantPayoutId,
