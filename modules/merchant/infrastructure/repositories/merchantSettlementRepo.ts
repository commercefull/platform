...
export async function create(
  params: Omit<MerchantSettlement, 'merchantSettlementId' | 'createdAt' | 'updatedAt'>,
): Promise<MerchantSettlement | null> {
  const now = new Date();
...
export async function findByMerchant(merchantId: string, limit = 20, offset = 0): Promise<MerchantSettlement[]> {
  return (
    (await query<MerchantSettlement[]>(
...
export async function updateStatus(merchantSettlementId: string, status: string): Promise<void> {
  const now = new Date();
  await query(`UPDATE "merchantSettlement" SET status = $1, "processedAt" = $2, "updatedAt" = $3 WHERE "merchantSettlementId" = $4`, [
    status,
...
export async function addLine(
  params: Omit<MerchantSettlementLine, 'merchantSettlementLineId' | 'createdAt'>,
): Promise<MerchantSettlementLine | null> {
...
export async function findLines(merchantSettlementId: string): Promise<MerchantSettlementLine[]> {
  return (
    (await query<MerchantSettlementLine[]>(
