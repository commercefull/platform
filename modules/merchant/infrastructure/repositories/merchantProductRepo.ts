...
export async function create(
  params: Omit<MerchantProduct, 'merchantProductId' | 'createdAt' | 'updatedAt' | 'deletedAt'>,
...
export async function update(
  merchantProductId: string,
  params: Partial<Omit<MerchantProduct, 'merchantProductId' | 'merchantId' | 'productId' | 'createdAt' | 'updatedAt' | 'deletedAt'>>,
): Promise<MerchantProduct | null> {
  const now = new Date();
...
export async function softDelete(merchantProductId: string): Promise<void> {
  await query(`UPDATE "merchantProduct" SET "deletedAt" = $1, "updatedAt" = $2 WHERE "merchantProductId" = $3`, [
    new Date(),
    new Date(),
    merchantProductId,
  ]);
