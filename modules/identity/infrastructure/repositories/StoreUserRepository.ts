...
export class StoreUserRepository implements IStoreUserRepository {
  ...
  async findByUserAndStore(userId: string, storeId: string): Promise<UserStoreAssignment | null> {
    const row = await queryOne<DbStoreUser>(`SELECT * FROM "${this.tableName}" WHERE "userId" = $1 AND "storeId" = $2`, [
      userId,
      storeId,
    ]);
  ...
  async save(assignment: UserStoreAssignment): Promise<UserStoreAssignment> {
    const existing = await queryOne<{ userStoreId: string }>(`SELECT "userStoreId" FROM "${this.tableName}" WHERE "userStoreId" = $1`, [
...
export default new StoreUserRepository();
