...
export const getAllPriceRules = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (currencyCode && regionCode) {
      ...
      const currencyRules = await currencyPriceRuleRepo.findByCurrencyCode(currencyCode as string, showInactive);

      // Filter by region code
      rules = currencyRules.filter(rule => rule.regionCode === regionCode);
    } else if (currencyCode) {
      rules = await currencyPriceRuleRepo.findByCurrencyCode(currencyCode as string, showInactive);
    } else {
      ...
      for (const currency of currencies) {
        const currencyRules = await currencyPriceRuleRepo.findByCurrencyCode(currency.code, showInactive);
