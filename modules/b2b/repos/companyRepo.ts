...
export async function getCompany(companyId: string): Promise<B2bCompany | null> {
  const row = await queryOne<Record<string, any>>(
    'SELECT * FROM "b2bCompany" WHERE "b2bCompanyId" = $1 AND "deletedAt" IS NULL',
    [companyId]
  );
...
export async function getCompanyByVat(vatNumber: string): Promise<B2bCompany | null> {
  const row = await queryOne<Record<string, any>>(
    'SELECT * FROM "b2bCompany" WHERE "vatNumber" = $1 AND "deletedAt" IS NULL',
    [vatNumber]
  );
  return row ? mapToCompany(row) : null;
}
...
export async function getCompanies(
  ...
  const countResult = await queryOne<{ count: string }>(
    `SELECT COUNT(*) as count FROM "b2bCompany" WHERE ${whereClause}`,
    params
  );
...
export async function saveCompany(company: Partial<B2bCompany> & { name?: string; b2bCompanyId?: string }): Promise<B2bCompany> {
  ...
  if (company.b2bCompanyId) {
    ...
    await query(
      ...
      [
        merged.name, merged.legalName, merged.registrationNumber, merged.vatNumber,
        merged.taxId, merged.dunsNumber, merged.status || 'pending',
        merged.companyType || 'corporation', merged.industry, merged.employeeCount,
        merged.annualRevenue, merged.creditLimit || 0, merged.availableCredit || 0,
        merged.paymentTermsDays || 30, merged.paymentTermsType || 'prepaid',
        merged.currency || 'USD', merged.website, merged.phone, merged.email,
        merged.logoUrl, merged.description, merged.notes, merged.tier || 'standard',
        merged.discountRate || 0, merged.requiresApproval || false,
        merged.orderMinimum, merged.orderMaximum, merged.taxExempt || false,
        merged.metadata ? JSON.stringify(merged.metadata) : null, now, company.b2bCompanyId
      ]
    );
    ...
    const result = await queryOne<Record<string, any>>(
      ...
      RETURNING *`,
      [
        company.name, company.legalName, company.registrationNumber, company.vatNumber,
        company.taxId, company.dunsNumber, 'pending', company.companyType || 'corporation',
        company.industry, company.employeeCount, company.annualRevenue, 0, 0,
        company.paymentTermsDays || 30, company.paymentTermsType || 'prepaid',
        company.currency || 'USD', company.website, company.phone, company.email,
        company.logoUrl, company.description, company.notes, 'standard', 0, false,
        company.orderMinimum, company.orderMaximum, company.taxExempt || false,
        company.metadata ? JSON.stringify(company.metadata) : null, now, now
      ]
    );
...
export async function suspendCompany(companyId: string): Promise<void> {
  await query(
    `UPDATE "b2bCompany" SET "status" = 'suspended', "updatedAt" = $1 WHERE "b2bCompanyId" = $2`,
...
export async function deleteCompany(companyId: string): Promise<void> {
  await query(
    'UPDATE "b2bCompany" SET "deletedAt" = $1 WHERE "b2bCompanyId" = $2',
    [new Date().toISOString(), companyId]
  );
}
...
export async function getCompanyUser(companyUserId: string): Promise<B2bCompanyUser | null> {
  const row = await queryOne<Record<string, any>>(
    'SELECT * FROM "b2bCompanyUser" WHERE "b2bCompanyUserId" = $1 AND "deletedAt" IS NULL',
    [companyUserId]
  );
...
export async function getCompanyUsers(companyId: string, includeInactive: boolean = false): Promise<B2bCompanyUser[]> {
  ...
  const rows = await query<Record<string, any>[]>(
    `SELECT * FROM "b2bCompanyUser" WHERE ${whereClause} ORDER BY "lastName", "firstName"`,
    [companyId]
  );
...
export async function saveCompanyUser(user: Partial<B2bCompanyUser> & { b2bCompanyId: string; email: string }): Promise<B2bCompanyUser> {
  ...
  if (user.b2bCompanyUserId) {
    await query(
      ...
      [
        user.firstName, user.lastName, user.phone, user.jobTitle, user.department,
        user.role || 'buyer', JSON.stringify(user.permissions || []), user.isActive !== false,
        user.isPrimaryContact || false, user.isBillingContact || false,
        user.canPlaceOrders !== false, user.canViewPrices !== false,
        user.canApproveOrders || false, user.canManageUsers || false,
        user.canManageCompany || false, user.orderLimit, user.monthlyLimit,
        user.requiresApproval || false, user.approverId, now, user.b2bCompanyUserId
      ]
    );
    ...
    const result = await queryOne<Record<string, any>>(
      ...
      RETURNING *`,
      [
        user.b2bCompanyId, user.customerId, user.email, user.firstName, user.lastName,
        user.phone, user.jobTitle, user.department, user.role || 'buyer',
        JSON.stringify(user.permissions || []), true, user.isPrimaryContact || false,
        user.isBillingContact || false, user.canPlaceOrders !== false,
        user.canViewPrices !== false, user.canApproveOrders || false,
        user.canManageUsers || false, user.canManageCompany || false,
        user.orderLimit, user.monthlyLimit, user.requiresApproval || false,
        user.approverId, inviteToken, now, inviteExpiresAt.toISOString(), now, now
      ]
    );
...
export async function deleteCompanyUser(companyUserId: string): Promise<void> {
  await query(
    'UPDATE "b2bCompanyUser" SET "deletedAt" = $1 WHERE "b2bCompanyUserId" = $2',
    [new Date().toISOString(), companyUserId]
  );
}
...
export async function saveCompanyAddress(address: Partial<B2bCompanyAddress> & {
  b2bCompanyId: string;
  addressLine1: string;
  city: string;
  countryCode: string;
  ...
  if (address.b2bCompanyAddressId) {
    await query(
      ...
      [
        address.addressType || 'shipping', address.label, address.contactName,
        address.contactPhone, address.contactEmail, address.company, address.addressLine1,
        address.addressLine2, address.addressLine3, address.city, address.state,
        address.postalCode, address.countryCode, address.country, address.isDefault || false,
        address.isDefaultBilling || false, address.isDefaultShipping || false,
        address.deliveryInstructions, address.metadata ? JSON.stringify(address.metadata) : null,
        now, address.b2bCompanyAddressId
      ]
    );
    ...
    const result = await queryOne<Record<string, any>>(
      ...
      RETURNING *`,
      [
        address.b2bCompanyId, address.addressType || 'shipping', address.label,
        address.contactName, address.contactPhone, address.contactEmail, address.company,
        address.addressLine1, address.addressLine2, address.addressLine3, address.city,
        address.state, address.postalCode, address.countryCode, address.country,
        address.isDefault || false, address.isDefaultBilling || false,
        address.isDefaultShipping || false, address.deliveryInstructions,
...
export async function deleteCompanyAddress(companyAddressId: string): Promise<void> {
  await query(
    'UPDATE "b2bCompanyAddress" SET "deletedAt" = $1 WHERE "b2bCompanyAddressId" = $2',
    [new Date().toISOString(), companyAddressId]
  );
}
