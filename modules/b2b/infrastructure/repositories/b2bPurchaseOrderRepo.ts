...
export async function findById(b2bPurchaseOrderId: string): Promise<B2bPurchaseOrder | null> {
  return queryOne<B2bPurchaseOrder>(`SELECT * FROM "b2bPurchaseOrder" WHERE "b2bPurchaseOrderId" = $1 AND "deletedAt" IS NULL`, [
    b2bPurchaseOrderId,
  ]);
}
...
export async function create(data: {
  ...
  return queryOne<B2bPurchaseOrder>(
    `INSERT INTO "b2bPurchaseOrder" ("b2bCompanyId", "orderNumber", "status", "currency", "totalAmount", "notes", "createdAt", "updatedAt")
     ...
     RETURNING *`,
    [data.b2bCompanyId, data.orderNumber ?? null, data.currency ?? 'USD', data.totalAmount ?? 0, data.notes ?? null, now],
  ) as Promise<B2bPurchaseOrder>;
}

export async function updateStatus(b2bPurchaseOrderId: string, status: B2bPurchaseOrderStatus): Promise<B2bPurchaseOrder | null> {
  const now = new Date().toISOString();
  return queryOne<B2bPurchaseOrder>(
...
export async function softDelete(b2bPurchaseOrderId: string): Promise<void> {
  await query(`UPDATE "b2bPurchaseOrder" SET "deletedAt" = $1, "updatedAt" = $1 WHERE "b2bPurchaseOrderId" = $2`, [
