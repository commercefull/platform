...
export async function getSalesSummary(
  ...
  const result = await queryOne<{
    ...
    `SELECT 
      COALESCE(SUM("netRevenueCents"), 0) as "totalRevenueCents",
      COALESCE(SUM("orderCount"), 0) as "totalOrders",
      CASE WHEN SUM("orderCount") > 0 
        THEN SUM("netRevenueCents") / SUM("orderCount") 
        ELSE 0 END as "averageOrderValueCents",
  ...
  return {
    totalRevenueCents: parseFloat(result?.totalRevenueCents || '0'),
    totalOrders: parseInt(result?.totalOrders || '0'),
    averageOrderValueCents: parseFloat(result?.averageOrderValueCents || '0'),
...
export async function upsertSalesDaily(
  ...
  await query(
    ...
    ON CONFLICT ("organizationId", "date", "channel", "currencyCode") DO UPDATE SET
      ...
      "discountTotalCents" = "analyticsSalesDaily"."discountTotalCents" + EXCLUDED."discountTotalCents",
      "refundTotalCents" = "analyticsSalesDaily"."refundTotalCents" + EXCLUDED."refundTotalCents",
      "netRevenueCents" = "analyticsSalesDaily"."netRevenueCents" + EXCLUDED."netRevenueCents",
      "taxTotalCents" = "analyticsSalesDaily"."taxTotalCents" + EXCLUDED."taxTotalCents",
      "shippingRevenueCents" = "analyticsSalesDaily"."shippingRevenueCents" + EXCLUDED."shippingRevenueCents",
    ...
    [
      ...
      data.refundTotalCents || 0,
      data.netRevenueCents || 0,
      data.taxTotalCents || 0,
      data.shippingRevenueCents || 0,
      data.averageOrderValueCents || 0,
      data.newCustomers || 0,
...
export async function getProductPerformance(
  ...
  const rows = await query<AnalyticsProductPerformanceRow[]>(
    `SELECT * FROM "analyticsProductPerformance" WHERE ${whereClause} 
     ORDER BY "date" DESC, "revenueCents" DESC LIMIT $${paramIndex++} OFFSET $${paramIndex}`,
    [...params, limit, offset],
  );
...
export async function getTopProducts(
  ...
  const rows = await query<AnalyticsProductPerformanceRow[]>(
    `SELECT "productId", 
      ...
      SUM("revenueCents") as "revenueCents",
      AVG("averageRating") as "averageRating"
     FROM "analyticsProductPerformance" 
     WHERE "date" >= $1 AND "date" <= $2
     GROUP BY "productId"
     ORDER BY SUM("${metric === 'revenue' ? 'revenueCents' : metric}") DESC
     LIMIT $3`,
...
export async function upsertProductPerformance(
  ...
  await query(
    ...
    ON CONFLICT ("productId", "productVariantId", "date", "channel") DO UPDATE SET
      ...
      "revenueCents" = "analyticsProductPerformance"."revenueCents" + EXCLUDED."revenueCents",
...
export async function upsertSearchQuery(data: {
  ...
  await query(
    `INSERT INTO "analyticsSearchQuery" (
      ...
      "clickCount", "purchaseCount", "revenueCents",
      "createdAt", "updatedAt"
    ...
    ON CONFLICT ("organizationId", "queryNormalized", "date") DO UPDATE SET
      ...
      "revenueCents" = "analyticsSearchQuery"."revenueCents" + EXCLUDED."revenueCents",
      "updatedAt" = $11`,
    [
      data.organizationId,
      data.query,
