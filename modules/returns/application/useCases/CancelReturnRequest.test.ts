...
describe('CancelReturnRequestUseCase', () => {
  ...
  it('should throw ReturnNotFoundError when the return does not exist', async () => {
    ...
    await expect(new CancelReturnRequestUseCase(repo).execute('missing')).rejects.toThrow(ReturnNotFoundError);
  ...
  it('should throw ReturnNotFoundError when the update does not persist', async () => {
    ...
    await expect(new CancelReturnRequestUseCase(repo).execute('r-1')).rejects.toThrow(ReturnNotFoundError);
    expect(emitMock).not.toHaveBeenCalled();
  });

  it('should throw InvalidReturnTransitionError when the return is already cancelled', async () => {
    const req = createReturnRequest();
    req.cancel();
    repo.findById.mockResolvedValue(req);

    await expect(new CancelReturnRequestUseCase(repo).execute('r-1')).rejects.toThrow(InvalidReturnTransitionError);
    expect(repo.update).not.toHaveBeenCalled();
  });
});
