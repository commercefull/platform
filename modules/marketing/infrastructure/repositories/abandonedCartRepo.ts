...
export async function create(
  params: Omit<MarketingAbandonedCart, 'marketingAbandonedCartId' | 'createdAt' | 'updatedAt'>,
): Promise<MarketingAbandonedCart | null> {
  const now = new Date();
  return queryOne<MarketingAbandonedCart>(
    ...
    [
      ...
      params.currency,
      params.status,
      params.recoveredAt || null,
      params.recoveredOrderId || null,
      now,
      now,
    ],
  );
...
export async function findPendingRecovery(limit = 100): Promise<MarketingAbandonedCart[]> {
  return (
    (await query<MarketingAbandonedCart[]>(
...
export async function markRecovered(id: string, orderId: string): Promise<void> {
  const now = new Date();
  await query(
    `UPDATE "marketingAbandonedCart" SET status = 'recovered', "recoveredAt" = $1, "recoveredOrderId" = $2, "updatedAt" = $3 WHERE "marketingAbandonedCartId" = $4`,
    [now, orderId, now, id],
  );
}
