...
export async function create(
  params: Omit<
    ...
    'marketingEmailCampaignId' | 'recipientCount' | 'openCount' | 'clickCount' | 'createdAt' | 'updatedAt'
  >,
): Promise<MarketingEmailCampaign | null> {
  const now = new Date();
...
export async function findByStatus(status: string, limit = 50): Promise<MarketingEmailCampaign[]> {
  return (
    (await query<MarketingEmailCampaign[]>(`SELECT * FROM "marketingEmailCampaign" WHERE status = $1 ORDER BY "createdAt" DESC LIMIT $2`, [
      status,
      limit,
    ])) || []
  );
...
export async function updateStatus(id: string, status: string): Promise<void> {
  await query(`UPDATE "marketingEmailCampaign" SET status = $1, "updatedAt" = $2 WHERE "marketingEmailCampaignId" = $3`, [
    status,
    new Date(),
    id,
  ]);
...
export async function incrementStats(id: string, field: 'openCount' | 'clickCount'): Promise<void> {
  await query(`UPDATE "marketingEmailCampaign" SET "${field}" = "${field}" + 1, "updatedAt" = $1 WHERE "marketingEmailCampaignId" = $2`, [
    new Date(),
    id,
  ]);
}
