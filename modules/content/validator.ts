...
export const userContactUsValidationRules = () => {
  return [
    ...
    check('message', 'Please enter a message with at least 10 words').not().isEmpty().isLength({ min: 10 }),
  ];
};

export const userContactFormValidationRules = () => {
  return [
    check('name', 'Please enter a valid name (minimum 2 characters)').not().isEmpty().isLength({ min: 2 }),
    check('email', 'Please enter a valid email address').not().isEmpty().isEmail(),
    check('subject', 'Please select a subject').not().isEmpty(),
    check('message', 'Please enter a message (minimum 10 characters)').not().isEmpty().isLength({ min: 10 }),
