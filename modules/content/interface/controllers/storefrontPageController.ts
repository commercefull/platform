import { logger } from '../../../../libs/logger';
import type { HttpRequest, HttpRequestBody, HttpResponse } from 'libs/http';
import { storefrontRespond } from '../../../../libs/storefrontRespond';
import { listProductsUseCase } from '../../../product/application/useCases/wired';
import { ListProductsCommand } from '../../../product/application/useCases/ListProducts';

// GET: home page
export const getHomePage = async (req: HttpRequest, res: HttpResponse): Promise<void> => {
  // Get featured/latest products
  const productCommand = new ListProductsCommand({ isFeatured: true }, 9, 0);
  const productResult = await listProductsUseCase.execute(productCommand);

  storefrontRespond(req, res, 'page/home', {
    pageName: 'Home',
    products: productResult.products,
    // Categories are not yet available via product module. Pass empty for now.
    categories: [],
    user: req.user,
  });
};

// GET: display about us page
export const getAboutUsPage = (req: HttpRequest, res: HttpResponse): void => {
  storefrontRespond(req, res, 'page/about-us', {
    pageName: 'About Us',
    user: req.user,
  });
};

// GET: display shipping policy page
export const getShippingPolicyPage = (req: HttpRequest, res: HttpResponse): void => {
  storefrontRespond(req, res, 'page/shipping-policy', {
    pageName: 'Shipping Policy',
    user: req.user,
  });
};

// GET: display careers page
export const getCareersPage = (req: HttpRequest, res: HttpResponse): void => {
  storefrontRespond(req, res, 'page/careers', {
    pageName: 'Careers',
    user: req.user,
  });
};

// GET: display contact us page and form with csrf tokens
export const getContactUsPage = (req: HttpRequest, res: HttpResponse): void => {
  const successMsg = req.flash('success')[0];
  const errorMsg = req.flash('error');
  storefrontRespond(req, res, 'page/contact-us', {
    pageName: 'Contact Us',
    successMsg,
    errorMsg,
    user: req.user,
  });
};

// POST: handle contact us form logic using nodemailer
export const submitContactForm = (req: HttpRequest, res: HttpResponse): void => {
  // This would typically send an email, but for now we'll just redirect with success
  req.flash('success', "Thank you for your message! We'll get back to you soon.");
  res.redirect('/pages/contact-us');
};

// GET: display contact form page
export const getContactFormPage = (req: HttpRequest, res: HttpResponse): void => {
  const successMsg = req.flash('success')[0];
  const errorMsg = req.flash('error');
  storefrontRespond(req, res, 'page/contact-form', {
    pageName: 'Contact Form',
    successMsg,
    errorMsg,
    user: req.user,
  });
};

// GET: display FAQ page
export const getFaqPage = (req: HttpRequest, res: HttpResponse): void => {
  const successMsg = req.flash('success')[0];
  const errorMsg = req.flash('error');
  storefrontRespond(req, res, 'page/faq', {
    pageName: 'FAQ',
    successMsg,
    errorMsg,
    user: req.user,
  });
};

// GET: display returns page
export const getReturnsPage = (req: HttpRequest, res: HttpResponse): void => {
  const successMsg = req.flash('success')[0];
  const errorMsg = req.flash('error');
  storefrontRespond(req, res, 'page/returns', {
    pageName: 'Returns & Exchanges',
    successMsg,
    errorMsg,
    user: req.user,
  });
};

// GET: display support page
export const getSupportPage = (req: HttpRequest, res: HttpResponse): void => {
  const successMsg = req.flash('success')[0];
  const errorMsg = req.flash('error');
  storefrontRespond(req, res, 'page/support', {
    pageName: 'Support Center',
    successMsg,
    errorMsg,
    user: req.user,
  });
};

// POST: handle contact form submission
export const submitContactFormAdvanced = (req: HttpRequest, res: HttpResponse): void => {
  // Validation is handled by middleware, so we can proceed with processing
  const body = req.body as HttpRequestBody;
  const { name, email, phone, subject, message } = body;

  // TODO: Add email sending logic here using nodemailer
  // For now, we'll just log the form data and show success
  logger.info('Advanced contact form submission', {
    name: (name as string).trim(),
    email: (email as string).trim(),
    phone: (phone as string)?.trim() || null,
    subject,
    message: (message as string).trim(),
    submittedAt: new Date(),
  });

  req.flash('success', "Thank you for your message! We've received your inquiry and will get back to you within 24 hours.");
  res.redirect('/contact-form');
};
