...
export class ImportJobRepositoryImpl implements ImportJobRepository {
  async create(job: ImportJob): Promise<ImportJob> {
    ...
    await query(
      ...
      [
        ...
        props.source,
        props.status,
        props.sourceStoreUrl,
        props.sourceApiKey,
        JSON.stringify(props.sourceConfig || {}),
  ...
  async findById(importJobId: string): Promise<ImportJob | null> {
    const row = await queryOne<ImportJobDbRow>(`SELECT * FROM "${Table.ImportJob}" WHERE "importJobId" = $1`, [importJobId]);
    if (!row) return null;
