...
export class StockReservationRepo {
  async findById(id: string): Promise<StockReservation | null> {
    return await queryOne<StockReservation>(`SELECT * FROM "inventoryStockReservation" WHERE "stockReservationId" = $1`, [id]);
  }

  async findByProduct(productId: string, warehouseId?: string): Promise<StockReservation[]> {
    let sql = `SELECT * FROM "inventoryStockReservation" WHERE "productId" = $1`;
  ...
  async findByReference(referenceType: string, referenceId: string): Promise<StockReservation[]> {
    return (
      (await query<StockReservation[]>(
        `SELECT * FROM "inventoryStockReservation" WHERE "referenceType" = $1 AND "referenceId" = $2 ORDER BY "createdAt" DESC`,
  ...
  async findExpired(): Promise<StockReservation[]> {
    return (
      (await query<StockReservation[]>(
        `SELECT * FROM "inventoryStockReservation" WHERE "expiresAt" IS NOT NULL AND "expiresAt" < $1 ORDER BY "expiresAt" ASC`,
        [unixTimestamp()],
      )) || []
    );
  }
  ...
  async update(id: string, params: StockReservationUpdateParams): Promise<StockReservation | null> {
    ...
    return await queryOne<StockReservation>(
      `UPDATE "inventoryStockReservation" SET ${updateFields.join(', ')} WHERE "stockReservationId" = $${paramIndex} RETURNING *`,
      values,
    );
  }
  ...
  async delete(id: string): Promise<boolean> {
    const result = await queryOne<{ stockReservationId: string }>(
      `DELETE FROM "inventoryStockReservation" WHERE "stockReservationId" = $1 RETURNING "stockReservationId"`,
      [id],
    );
    return !!result;
  }
  ...
  async deleteExpired(): Promise<number> {
    const results = await query<{ stockReservationId: string }[]>(
      `DELETE FROM "inventoryStockReservation" WHERE "expiresAt" IS NOT NULL AND "expiresAt" < $1 RETURNING "stockReservationId"`,
      [unixTimestamp()],
    );
  ...
  async getTotalReserved(productId: string, warehouseId?: string, productVariantId?: string): Promise<number> {
    let sql = `SELECT SUM("quantity") as total FROM "inventoryStockReservation" WHERE "productId" = $1`;
    const params: unknown[] = [productId];
  ...
  async count(productId?: string): Promise<number> {
    let sql = `SELECT COUNT(*) as count FROM "inventoryStockReservation"`;
