...
import { FailoverRoutingEngine, GatewayRoute } from './FailoverRoutingEngine';
import {
  PSPAdapter,
  PSPCapabilities,
  PSPConfig,
...
describe('FailoverRoutingEngine', () => {
  ...
  it('should route to the highest priority provider', async () => {
    const result = await engine.routePayment(paymentRequest, routes());
  ...
  it('should trip circuit breaker after threshold failures', async () => {
    ...
    await engine.routePayment(paymentRequest, r);
    ...
    const result = await engine.routePayment(paymentRequest, r);
    expect(result.provider).toBe('secondary');
  });

  it('should persist circuit breaker state across calls with different route tables', async () => {
    primary.shouldThrow = true;

    // Trip the breaker for 'primary' (3 failures at threshold 3)
    for (let i = 0; i < 3; i++) {
      await engine.routePayment(paymentRequest, routes());
    }

    primary.shouldThrow = false;
    // A fresh route table (e.g. rebuilt for another organization) still sees
    // the persisted breaker — 'primary' must be skipped without an attempt
    const result = await engine.routePayment(paymentRequest, routes());
    expect(result.provider).toBe('secondary');
    expect(result.attempts.every(a => a.provider === 'secondary')).toBe(true);
  });
  ...
  it('should throw when no routes are provided', async () => {
    await expect(engine.routePayment(paymentRequest, [])).rejects.toThrow('No payment gateway configured');
  });
  ...
  it('should throw when all providers have tripped circuit breakers', async () => {
    ...
    await expect(engine.routePayment(paymentRequest, r)).rejects.toThrow('All payment providers are unavailable');
  });
