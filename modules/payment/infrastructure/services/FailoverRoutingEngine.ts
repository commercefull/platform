...
export class FailoverRoutingEngine {
  ...
  registerRoutes(routes: GatewayRoute[]): void {
    ...
    for (const route of routes) {
      this.ensureBreaker(route.provider);
    }
  }
   ...
   * Routes are passed per-call so per-organization route tables never race;
   * circuit breaker state is keyed by provider and persists across calls.
   */
  async routePayment(request: PaymentRequest, routes: GatewayRoute[]): Promise<RoutingResult> {
    if (routes.length === 0) {
    ...
    const availableRoutes = this.getAvailableRoutes(sortedRoutes);
  ...
  private ensureBreaker(provider: string): CircuitBreakerState {
    let state = this.circuitBreakers.get(provider);
    if (!state) {
      state = { failures: 0, lastFailureAt: 0, tripped: false };
      this.circuitBreakers.set(provider, state);
    }
    return state;
  }

  private getAvailableRoutes(routes: GatewayRoute[]): GatewayRoute[] {
    const now = Date.now();
