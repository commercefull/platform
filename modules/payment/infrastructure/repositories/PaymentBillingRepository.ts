...
// schema (e.g. "currencyCode" -> currency, "availableAmount" -> amountCents).
const BALANCE_COLUMNS = `"paymentBalanceId", "organizationId", "currencyCode" AS currency, "availableAmountCents" AS "amountCents", "createdAt", "updatedAt"`;
const DISPUTE_COLUMNS = `"paymentDisputeId", "orderPaymentId" AS "paymentId", "organizationId", "gatewayDisputeId" AS "externalDisputeId", status, reason, "amountCents", "currencyCode" AS currency, "evidenceDetails" AS evidence, "evidenceDueBy" AS "dueBy", "resolvedAt", "createdAt", "updatedAt"`;
const FEE_COLUMNS = `"paymentFeeId", "orderPaymentId" AS "transactionId", "organizationId", type, "amountCents", "currencyCode" AS currency, description, "createdAt", "updatedAt"`;
const REPORT_COLUMNS = `"paymentReportId", "organizationId", type, "parameters"->>'currency' AS currency, COALESCE(("parameters"->>'totalAmountCents')::numeric, 0)::float8 AS "totalAmountCents", COALESCE(("parameters"->>'transactionCount')::int, 0) AS "transactionCount", "parameters" AS data, lower("dateRange") AS "periodStart", upper("dateRange") AS "periodEnd", "createdAt", "updatedAt"`;
...
class PaymentBillingRepo {
  ...
  async creditBalance(organizationId: string, currency: string, amountCents: number): Promise<PaymentBalance | null> {
    const now = new Date();
    return queryOne<PaymentBalance>(
      `INSERT INTO "paymentBalance" ("organizationId", "currencyCode", "availableAmountCents", "createdAt", "updatedAt")
       VALUES ($1, $2, $3, $4, $5)
       ON CONFLICT ("organizationId", "currencyCode") DO UPDATE SET "availableAmountCents" = "paymentBalance"."availableAmountCents" + $3, "updatedAt" = $5
       RETURNING ${BALANCE_COLUMNS}`,
      [organizationId, currency, amountCents, now, now],
    );
  }

  async debitBalance(organizationId: string, currency: string, amountCents: number): Promise<PaymentBalance | null> {
    const now = new Date();
    return queryOne<PaymentBalance>(
      `UPDATE "paymentBalance" SET "availableAmountCents" = "availableAmountCents" - $1, "updatedAt" = $2
       WHERE "organizationId" = $3 AND "currencyCode" = $4
       RETURNING ${BALANCE_COLUMNS}`,
      [amountCents, now, organizationId, currency],
    );
  }

  async getBalance(organizationId: string, currency: string): Promise<number> {
    const result = await queryOne<{ amountCents: string }>(
      `SELECT COALESCE("availableAmountCents", 0) AS "amountCents" FROM "paymentBalance" WHERE "organizationId" = $1 AND "currencyCode" = $2`,
      [organizationId, currency],
    );
    return result ? Number(result.amountCents) : 0;
  }
  ...
  async createDispute(params: Omit<PaymentDispute, 'paymentDisputeId' | 'createdAt' | 'updatedAt'>): Promise<PaymentDispute | null> {
    ...
    return queryOne<PaymentDispute>(
      `INSERT INTO "paymentDispute" ("orderPaymentId", "organizationId", "gatewayDisputeId", status, reason, amountCents, "currencyCode", "evidenceDetails", "evidenceDueBy", "resolvedAt", "createdAt", "updatedAt")
  ...
  async createFee(params: Omit<PaymentFee, 'paymentFeeId' | 'createdAt' | 'updatedAt'>): Promise<PaymentFee | null> {
    ...
    return queryOne<PaymentFee>(
      `INSERT INTO "paymentFee" ("orderPaymentId", "organizationId", type, amountCents, "currencyCode", description, "createdAt", "updatedAt")
  ...
  async sumFeesByMerchant(organizationId: string, currency: string): Promise<number> {
    const result = await queryOne<{ total: string }>(
      `SELECT COALESCE(SUM(amountCents), 0) AS total FROM "paymentFee" WHERE "organizationId" = $1 AND "currencyCode" = $2`,
      [organizationId, currency],
    );
