...
export class PayoutItemRepo {
  async findById(id: string): Promise<PayoutItem | null> {
    return await queryOne<PayoutItem>(`SELECT * FROM "paymentPayoutItem" WHERE "payoutItemId" = $1 AND "deletedAt" IS NULL`, [id]);
  }

  async findByPayout(payoutId: string): Promise<PayoutItem[]> {
    return (
      (await query<PayoutItem[]>(`SELECT * FROM "paymentPayoutItem" WHERE "payoutId" = $1 AND "deletedAt" IS NULL ORDER BY "createdAt" ASC`, [
        payoutId,
      ])) || []
    );
  }
  ...
  async findByType(payoutId: string, type: PayoutItemType): Promise<PayoutItem[]> {
    return (
      (await query<PayoutItem[]>(
        `SELECT * FROM "paymentPayoutItem" WHERE "payoutId" = $1 AND "type" = $2 AND "deletedAt" IS NULL ORDER BY "createdAt" ASC`,
        [payoutId, type],
      )) || []
    );
  }
  ...
  async findByOrder(orderId: string): Promise<PayoutItem[]> {
    return (
      (await query<PayoutItem[]>(`SELECT * FROM "paymentPayoutItem" WHERE "orderId" = $1 AND "deletedAt" IS NULL ORDER BY "createdAt" DESC`, [
        orderId,
      ])) || []
    );
  }
  ...
  async delete(id: string): Promise<boolean> {
    const result = await queryOne<{ payoutItemId: string }>(
      `UPDATE "paymentPayoutItem" SET "deletedAt" = $1 WHERE "payoutItemId" = $2 AND "deletedAt" IS NULL RETURNING "payoutItemId"`,
      [unixTimestamp(), id],
    );
    return !!result;
  }
  ...
  async deleteByPayout(payoutId: string): Promise<number> {
    const results = await query<{ payoutItemId: string }[]>(
      `UPDATE "paymentPayoutItem" SET "deletedAt" = $1 WHERE "payoutId" = $2 AND "deletedAt" IS NULL RETURNING "payoutItemId"`,
  ...
  async getTotalAmount(payoutId: string): Promise<number> {
    const result = await queryOne<{ total: string }>(
      `SELECT SUM("amount") as total FROM "paymentPayoutItem" WHERE "payoutId" = $1 AND "deletedAt" IS NULL`,
      [payoutId],
    );
  ...
  async count(payoutId?: string): Promise<number> {
    let sql = `SELECT COUNT(*) as count FROM "paymentPayoutItem" WHERE "deletedAt" IS NULL`;
