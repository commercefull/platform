...
export class PaymentRepo {
  ...
  async findAllPaymentMethods(): Promise<PaymentMethod[]> {
    const methods = await query<PaymentMethod[]>('SELECT * FROM "paymentMethod" WHERE "deleted_at" IS NULL ORDER BY "name" ASC');
    return methods || [];
  }

  async findPaymentMethodById(id: string): Promise<PaymentMethod | null> {
    const method = await queryOne<PaymentMethod>('SELECT * FROM "paymentMethod" WHERE "id" = $1 AND "deleted_at" IS NULL', [id]);
    return method || null;
  }
  ...
  async findActivePaymentMethods(): Promise<PaymentMethod[]> {
    const methods = await query<PaymentMethod[]>(
      'SELECT * FROM "paymentMethod" WHERE "is_active" = true AND "deleted_at" IS NULL ORDER BY "name" ASC',
    );
    return methods || [];
  }
  ...
  async findActivePaymentGateways(): Promise<PaymentGateway[]> {
    const gateways = await query<PaymentGateway[]>(
      'SELECT * FROM "paymentGateway" WHERE "is_active" = true AND "deleted_at" IS NULL ORDER BY "name" ASC',
    );
    return gateways || [];
  }
