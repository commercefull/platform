...
describe('OrderRouter', () => {
  ...
  it('should throw when no eligible stores', async () => {
    const noMatchStores = [
      { storeId: 's3', name: 'Store 3', canFulfillOnline: false, canPickupInStore: false, localDeliveryEnabled: false },
    ];
  ...
  it('should determine pickup store by distance', async () => {
    mockInventoryRepo.getAvailableQuantity.mockResolvedValue(5);
    const result = await router.determinePickupStore(order, { latitude: 40.7, longitude: -74.0 }, stores);
    expect(result.storeId).toBe('s1');
  });
  ...
  it('should throw when no fulfillment stores', async () => {
    await expect(
      router.determineFulfillmentStore(order, [stores[1].storeId === 's2' ? { ...stores[1], canFulfillOnline: false } : stores[1]]),
    ).rejects.toThrow(NoFulfillmentStoresError);
  });
