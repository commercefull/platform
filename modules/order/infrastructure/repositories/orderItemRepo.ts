...
export class OrderItemRepo {
  ...
  async findById(orderItemId: string): Promise<OrderItem | null> {
    return await queryOne<OrderItem>(`SELECT * FROM "orderItem" WHERE "orderItemId" = $1`, [orderItemId]);
  }
  ...
  async findByOrderId(orderId: string): Promise<OrderItem[]> {
    const results = await query<OrderItem[]>(`SELECT * FROM "orderItem" WHERE "orderId" = $1 ORDER BY "createdAt" ASC`, [orderId]);
    return results || [];
  }
  ...
  async countByOrderId(orderId: string): Promise<number> {
    const result = await queryOne<{ count: string }>(`SELECT COUNT(*) as count FROM "orderItem" WHERE "orderId" = $1`, [orderId]);
