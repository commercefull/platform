...
export class ProductMediaRepo {
  ...
  async findById(productMediaId: string): Promise<ProductMedia | null> {
    return await queryOne<ProductMedia>(`SELECT * FROM "productMedia" WHERE "productMediaId" = $1`, [productMediaId]);
  }
  ...
  async delete(productMediaId: string): Promise<boolean> {
    const result = await queryOne<{ productMediaId: string }>(
      `DELETE FROM "productMedia" WHERE "productMediaId" = $1 RETURNING "productMediaId"`,
      [productMediaId],
    );
  ...
  async deleteByVariantId(productVariantId: string): Promise<number> {
    const result = await queryOne<{ count: string }>(
      `DELETE FROM "productMedia" WHERE "productVariantId" = $1 RETURNING COUNT(*) as count`,
      [productVariantId],
    );
  ...
  async countByType(type: MediaType): Promise<number> {
    const result = await queryOne<{ count: string }>(`SELECT COUNT(*) as count FROM "productMedia" WHERE "type" = $1`, [type]);
  ...
  async getStatistics(): Promise<Record<MediaType, number>> {
    const results = await query<{ type: MediaType; count: string }[]>(
      `SELECT "type", COUNT(*) as count FROM "productMedia" GROUP BY "type"`,
      [],
    );
  ...
  async getTotalStorageSize(): Promise<number> {
    const result = await queryOne<{ total: string }>(`SELECT COALESCE(SUM("filesize"), 0) as total FROM "productMedia"`, []);
