...
describe('EvaluatePromotionsUseCase', () => {
  let useCase: EvaluatePromotionsUseCase;
  let promotionsRepo: jest.Mocked<PromotionsPort>;

  beforeEach(() => {
    jest.clearAllMocks();
    promotionsRepo = lazyMock<PromotionsPort>();
    useCase = new EvaluatePromotionsUseCase(promotionsRepo);
  });
  ...
  it('should apply coupon code promotions first', async () => {
    ...
    const result = await useCase.execute({ ...baseContext, couponCode: 'save10' });
  ...
  it('should evaluate customerGroup rule', async () => {
    ...
    const result = await useCase.execute({ ...baseContext, customerGroup: 'vip' });
    expect(result.totalDiscountAmountCents).toBe(20);
  });
  ...
  it('should evaluate firstOrder rule', async () => {
    ...
    const result = await useCase.execute({ ...baseContext, isFirstOrder: true });
    expect(result.totalDiscountAmountCents).toBe(20);
  });
  ...
  it('should evaluate shippingMethod rule', async () => {
    ...
    const result = await useCase.execute({ ...baseContext, shippingMethodId: 'sm1' });
    expect(result.totalDiscountAmountCents).toBe(20);
  });
  ...
  it('should evaluate paymentMethod rule', async () => {
    ...
    const result = await useCase.execute({ ...baseContext, paymentMethodId: 'pm1' });
    expect(result.totalDiscountAmountCents).toBe(20);
  });
  ...
  it('should handle empty items', async () => {
    promotionsRepo.findActive.mockResolvedValue([]);
    const result = await useCase.execute({ items: [], subtotalCents: 0, shippingAmountCents: 0, currency: 'USD' });
    expect(result.totalDiscountAmountCents).toBe(0);
  });
  ...
  describe('Epic C — freeGift action', () => {
    ...
    it('should not add free gift when minCartTotal not met', async () => {
      ...
      const result = await useCase.execute(baseContext); // subtotalCents 200 < 500
    ...
    it('should not add free gift when minQuantity not met', async () => {
      ...
      const result = await useCase.execute(baseContext); // qty 3 < 10
      expect(result.freeItems).toHaveLength(0);
    });
