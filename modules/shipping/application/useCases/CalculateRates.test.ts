...
describe('CalculateRatesUseCase', () => {
  ...
  beforeEach(() => {
    ...
    repo.findMethodsForZones.mockResolvedValue([
      createShippingMethodEntity({ shippingMethodId: 'm1', basePrice: 9.99, isDefault: true }),
      createShippingMethodEntity({ shippingMethodId: 'm2', name: 'Express', code: 'exp', basePrice: 19.99, isDefault: false }),
    ]);
  ...
  it('should skip methods unavailable for the order weight', async () => {
    repo.findMethodsForZones.mockResolvedValue([
      ...
      createShippingMethodEntity({ shippingMethodId: 'm2', code: 'exp', basePrice: 19.99 }),
    ]);
  ...
  it('should use the cheapest rate as default when no method is marked default', async () => {
    repo.findMethodsForZones.mockResolvedValue([
      createShippingMethodEntity({ shippingMethodId: 'm1', basePrice: 9.99, isDefault: false }),
      createShippingMethodEntity({ shippingMethodId: 'm2', code: 'exp', basePrice: 19.99, isDefault: false }),
    ]);
