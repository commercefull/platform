...
export async function findActive(): Promise<FulfillmentRule[]> {
  return (await query<FulfillmentRule[]>(`SELECT * FROM "fulfillmentRule" WHERE "isActive" = true ORDER BY priority ASC`)) || [];
}
...
export async function create(
  params: Omit<FulfillmentRule, 'fulfillmentRuleId' | 'createdAt' | 'updatedAt'>,
): Promise<FulfillmentRule | null> {
  const now = new Date();
...
export async function updatePriority(fulfillmentRuleId: string, priority: number): Promise<void> {
  await query(`UPDATE "fulfillmentRule" SET priority = $1, "updatedAt" = $2 WHERE "fulfillmentRuleId" = $3`, [
    priority,
    new Date(),
    fulfillmentRuleId,
  ]);
