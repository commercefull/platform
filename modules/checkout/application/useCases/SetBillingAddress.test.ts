...
describe('SetBillingAddressUseCase', () => {
  ...
  it('should set billing address (happy path)', async () => {
    const result = await useCase.execute(new SetBillingAddressCommand('ck-1', 'John', 'Doe', '123 Main St', 'Portland', '97201', 'US'));
  ...
  it('should throw CheckoutSessionNotFoundError when session does not exist', async () => {
    ...
    await expect(useCase.execute(new SetBillingAddressCommand('missing', 'John', 'Doe', '123 St', 'City', '12345', 'US'))).rejects.toThrow(
