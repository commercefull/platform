...
validate_environment() {
    ...
    if ! command -v gcloud &> /dev/null; then
        log_error "Google Cloud SDK not found. Please install gcloud CLI."
        exit 1
    fi
...
build_and_push_image() {
    ...
    if [[ -z "$project_id" ]]; then
        log_error "Could not determine project ID from terraform.tfvars"
        exit 1
    fi
    ...
    # Build image
    docker build -t "gcr.io/$project_id/commercefull:latest" "$PROJECT_ROOT"

    # Push image
    docker push "gcr.io/$project_id/commercefull:latest"
...
Cloud Run URL: ${cloud_run_url:-Not available}
Load Balancer IP: ${lb_ip:-Not available}

Next steps:
1. Update your DNS to point your domain to the load balancer IP
2. Run database migrations: gcloud run jobs create commercefull-migrate --image=gcr.io/$PROJECT_ID/commercefull:latest --set-env-vars=NODE_ENV=prod --set-cloudsql-instances=<cloudsql-connection-name> --task-timeout=300s && gcloud run jobs execute commercefull-migrate
3. Configure your application environment variables
4. Set up monitoring and alerts
5. Test the application: curl https://yourdomain.com/health

Useful GCP commands:
- View service: gcloud run services describe commercefull-app --region=us-central1
- View logs: gcloud logging read "resource.type=cloud_run_revision"
- SSH to instance: gcloud compute ssh --zone=us-central1-a INSTANCE_NAME
