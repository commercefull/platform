...
2. Run database migrations: az containerapp job create --name commercefull-migrate --resource-group commercefull-rg-${ENVIRONMENT} --image ${acr_name}.azurecr.io/commercefull:latest --set-env-vars NODE_ENV=${ENVIRONMENT} --command "yarn db:migrate"
