...
2. Run database migrations: aws ecs run-task --cluster commercefull-cluster --task-definition commercefull-${ENVIRONMENT} --count 1 --launch-type FARGATE --network-configuration "awsvpcConfiguration={subnets=[<subnet-ids>],securityGroups=[<sg-id>],assignPublicIp=DISABLED}" --overrides '{"containerOverrides":[{"name":"commercefull","command":["yarn","db:migrate"]}]}' --env NODE_ENV=${ENVIRONMENT}
