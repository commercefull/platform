...
exports.seed = async function (knex) {
  ...
  const products = await knex('product').whereIn('productId', [TEST_PRODUCT_1_ID, TEST_PRODUCT_2_ID]).select('productId');
  ...
  await knex('productBundleItem')
    .where('productBundleId', SEEDED_BUNDLE_1_ID)
    .delete()
    .catch(() => {});
  await knex('productBundle')
    .where('productBundleId', SEEDED_BUNDLE_1_ID)
    .delete()
    .catch(() => {});
  await knex('productQa')
    .where('productQaId', SEEDED_QA_1_ID)
    .delete()
    .catch(() => {});
  await knex('productReview')
    .where('productReviewId', SEEDED_REVIEW_1_ID)
    .delete()
    .catch(() => {});
