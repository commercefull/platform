...
class BasketPublicController {
  // Add item to cart
  async addToCart(req: any, res: Response): Promise<void> {
  ...
  // View shopping cart
  async viewCart(req: any, res: Response): Promise<void> {
    try {
      let basket;
      ...
      if (req.user && basket) {
        ...
        return storefrontRespond(req, res, 'basket/basket', {
          ...
          products: await this.productsFromBasket(basket)
        });
      }
      ...
      return storefrontRespond(req, res, 'basket/basket', {
        ...
        pageName: 'Shopping Cart',
        products: await this.productsFromBasket(req.session.cart)
      });
    } catch (err) {
  ...
  async reduceItem(req: any, res: Response): Promise<void> {
    const productId = req.params.id;
    try {
      let basketId;
  ...
  // View checkout form
  async viewCheckout(req: any, res: Response): Promise<void> {
    if (!req.session.cart) {
  ...
  private async productsFromBasket(basket: Basket): Promise<any[]> {
...
export default new BasketPublicController();
