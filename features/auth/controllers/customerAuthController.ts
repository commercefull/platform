...
export class CustomerAuthController {
  ...
  private createToken(payload: JwtPayload, secret: string, options: SignOptions): string {
  ...
  async generateToken(req: Request, res: Response): Promise<void> {
    try {
      ...
      await this.authRepo.saveRefreshToken(customer.id, 'customer', refreshToken);
  ...
  async refreshToken(req: Request, res: Response): Promise<void> {
    try {
      ...
      const isValidToken = await this.authRepo.verifyRefreshToken(payload.id, 'customer', refreshToken);
      if (!isValidToken) {
        res.status(401).json({ success: false, message: 'Refresh token has been revoked' });
        return;
      }

      const customer = await this.customerRepo.findCustomerById(payload.id);
      if (!customer) {
  ...
  async validateTokenEndpoint(req: Request, res: Response): Promise<void> {
    try {
