...
export class MerchantAuthController {
  ...
  async generateToken(req: Request, res: Response): Promise<void> {
    try {
      ...
      await this.authRepo.saveRefreshToken(merchant.id, 'merchant', refreshToken);
  ...
  async refreshToken(req: Request, res: Response): Promise<void> {
    try {
      ...
      const isValidToken = await this.authRepo.verifyRefreshToken(payload.id, 'merchant', refreshToken);
      if (!isValidToken) {
        res.status(401).json({ success: false, message: 'Refresh token has been revoked' });
        return;
      }

      const merchant = await this.merchantRepo.findById(payload.id);
      if (!merchant) {
