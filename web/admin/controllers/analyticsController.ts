...
import { getAnalyticsDataUseCase } from '../../../modules/analytics/application/wired';
import { generateReportUseCase, manageReportSchedulesUseCase } from '../../../modules/analytics/application/useCases';
import { predictiveAnalyticsUseCase } from '../../../modules/analytics/application/useCases/PredictiveAnalytics';
...
export const analyticsDashboard = async (req: TypedRequest, res: Response): Promise<void> => {
  ...
  // Get sales summary
  const salesSummary = await getAnalyticsDataUseCase.getSalesSummary(startDate, endDate);

  // Get top products
  const topProducts = await getAnalyticsDataUseCase.getTopProducts(startDate, endDate, 'revenue', 10);

  // Get customer cohorts for retention analysis
  const _customerCohorts = await getAnalyticsDataUseCase.getCustomerCohorts();
...
export const predictiveAnalytics = async (req: TypedRequest, res: Response): Promise<void> => {
  ...
  const forecasts = await predictiveAnalyticsUseCase.forecastSalesRevenue(historicalData, 30);
...
export const customerAnalytics = async (req: TypedRequest, res: Response): Promise<void> => {
  ...
  const segmentationAnalysis = await predictiveAnalyticsUseCase.performCustomerSegmentation();
...
export const aiRecommendations = async (req: TypedRequest, res: Response): Promise<void> => {
  ...
  if (sampleCustomerId) {
    productRecommendations = await predictiveAnalyticsUseCase.generateProductRecommendations(sampleCustomerId);
  }
...
export const createReportSchedule = async (req: TypedRequest, res: Response): Promise<void> => {
  ...
  const schedule = await manageReportSchedulesUseCase.scheduleReport({
    name,
