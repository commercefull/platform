...
exports.up = function (knex) {
  return knex.schema.createTable('inventoryTransaction', t => {
    ...
    t.uuid('typeId').notNullable().references('id').inTable('inventoryTransactionType').index('typeId');
    t.uuid('warehouseId').notNullable().references('id').inTable('warehouse').index('warehouseId');
    t.uuid('binId').references('id').inTable('warehouse_bin').index('binId');
    t.uuid('productId').notNullable().index('productId');
    t.uuid('variantId').index('variantId');
...
exports.down = function (knex) {
  return knex.schema.dropTable('inventory_transaction')
    .then(() => knex.schema.raw('DROP TYPE IF EXISTS inventory_transaction_status_type'));
};
