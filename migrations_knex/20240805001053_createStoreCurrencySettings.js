...
exports.up = function (knex) {
  return knex.schema.createTable('storeCurrencySettings', t => {
    ...
    t.uuid('storeCurrencyId').notNullable().references('id').inTable('currency');
    t.uuid('baseCurrencyId').notNullable().references('id').inTable('currency');
    t.uuid('displayCurrencyId').notNullable().references('id').inTable('currency');
...
exports.down = function (knex) {
  return knex.schema.dropTable('storeCurrencySettings')
    .then(() => knex.schema.raw('DROP TYPE IF EXISTS rounding_method_enum'))
    .then(() => knex.schema.raw('DROP TYPE IF EXISTS price_display_format_enum'));
};
