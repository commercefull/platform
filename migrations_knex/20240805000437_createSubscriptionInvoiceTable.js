exports.up = function(knex) {
  return knex.schema.createTable('subscriptionInvoice', t => {
    t.uuid('subscriptionInvoiceId').primary().defaultTo(knex.raw('uuid_generate_v4()'));
    t.uuid('subscriptionId').notNullable().references('paymentSubscriptionId').inTable('paymentSubscription').onDelete('CASCADE');
    ...
    t.uuid('paymentId').references('orderPaymentId').inTable('orderPayment');
