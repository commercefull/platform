exports.up = function(knex) {
  return knex.schema.createTable('cart_promotion', t => {
    t.uuid('cartPromotionId').primary().defaultTo(knex.raw('uuid_generate_v4()'));
    t.timestamp('createdAt').notNullable().defaultTo(knex.fn.now());
    t.timestamp('updatedAt').notNullable().defaultTo(knex.fn.now());
    t.uuid('cartId').notNullable().references('cartId').inTable('basket').onDelete('CASCADE');
    t.uuid('promotionId').notNullable().references('promotionId').inTable('promotion').onDelete('CASCADE');
    t.uuid('couponId').references('couponId').inTable('coupon').onDelete('SET NULL');
