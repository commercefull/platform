/**
 * Distribution Shipping Method Migration
 * Creates the distributionShippingMethod table for shipping method configuration
 */
exports.up = async function (knex) {
  return knex.schema.createTable('distributionShippingMethod', t => {
    t.uuid('distributionShippingMethodId').primary().defaultTo(knex.raw('uuidv7()'));
    t.timestamp('createdAt').notNullable().defaultTo(knex.fn.now());
    t.timestamp('updatedAt').notNullable().defaultTo(knex.fn.now());
    t.uuid('distributionShippingCarrierId').references('distributionShippingCarrierId').inTable('distributionShippingCarrier');
    ...
    t.uuid('createdBy');
    t.index('distributionShippingCarrierId');
    t.index('code');
    t.index('isActive');
    t.index('isDefault');
...
exports.down = async function (knex) {
  await knex.schema.dropTable('distributionShippingMethod');
};
