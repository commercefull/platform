...
exports.up = function(knex) {
  return knex.schema.createTable('distributionOrderFulfillment', (table) => {
    ...
    // Status tracking
    table.enum('status', ['pending', 'processing', 'picking', 'packing', 'ready_to_ship', 'shipped', 'in_transit', 'out_for_delivery', 'delivered', 'failed', 'returned', 'cancelled'])
    ...
    table.uuid('distributionWarehouseId').nullable()
      ...
      .onDelete('SET NULL');
    table.uuid('distributionRuleId').nullable()
      .references('distributionRuleId').inTable('distributionRule')
      .onDelete('SET NULL');
